...
async function contactusStore(req,res) {
    try {
        var data  = await ContactUs(req.body)
        await data.save()
...
async function newsletter(req,res) {
    try {
        let data = await Newsletter(req.body)
        await data.save()
...
async function newsletterConfirmation(req,res) {
    res.render("newsletter-confirmation",{title:"Newsletter Has Been Subcription Successfully"})
}


function errorPage(req, res) {
    res.render("404", { title: "404! Page Not Found" ,  session:req.session,})
}
